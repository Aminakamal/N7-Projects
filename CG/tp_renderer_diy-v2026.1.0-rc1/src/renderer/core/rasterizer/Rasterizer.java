...
public class Rasterizer {
    ...
    private void interpolate2(Fragment v1, Fragment v2, Fragment f) {
        ...
        // alpha repère la position relative du pixel sur le segment [v1, v2].
        // On interpole ensuite tous les attributs par combinaison affine.
    ...
    static double triangleArea(Fragment v1, Fragment v2, Fragment v3) {
        // Retourne 2 * aire orientée du triangle.
        // Sert à construire la matrice des coordonnées barycentriques.
    ...
    protected static Matrix makeBarycentricCoordsMatrix(final Fragment v1,
        ...
        // - alpha, beta, gamma servent à tester l'intérieur et à interpoler
        // Si bar = C * [1 x y]^T, alors bar = (alpha, beta, gamma).
    ...
    public void rasterizeFace(final Fragment v1, final Fragment v2, final Fragment v3)
        ...
        // - test inside triangle via barycentriques >= 0
        // - interpolation affine de tous les attributs
        // - clamp des couleurs dans [0, 1]

        // Triangle dégénéré: rien à rasterizer.
        ...
        // On parcourt seulement la bounding box du triangle pour limiter le coût.
        ...
        // Petite tolérance numérique pour éviter de perdre des pixels de bord.
        ...
        for (int x = xmin; x <= xmax; x++) {
            for (int y = ymin; y <= ymax; y++) {
                ...
                // bar = (alpha, beta, gamma) du pixel courant dans le triangle.
                final Vector barycentricCoords = cMat.multiply(
                        new Vector(1.0, (double) x, (double) y));

                // Un pixel est à l'intérieur si alpha, beta, gamma >= 0
                // (à epsilon près pour la robustesse numérique).
                if (barycentricCoords.get(0) < -eps
                        || barycentricCoords.get(1) < -eps
                        || barycentricCoords.get(2) < -eps) {
                    continue;
                }

                // Interpolation affine de tous les attributs du fragment:
                // depth, couleur, normale, UV, etc.
                for (int i = 0; i < numAttributes; i++) {
                    double interpolated = barycentricCoords.get(0) * v1.getAttribute(i)
                            + barycentricCoords.get(1) * v2.getAttribute(i)
                            + barycentricCoords.get(2) * v3.getAttribute(i);
                    if (i >= Fragment.COLOR_R && i <= Fragment.COLOR_B) {
                        // Clamp utile pour éviter des couleurs hors [0, 1]
                        // à cause des approximations numériques.
                        interpolated = MathUtils.clamp(interpolated, 0., 1.);
                    }
                    fragment.setAttribute(i, interpolated);
                }
                // Le shader fera la couleur finale et éventuellement le depth test.
                shader.shade(fragment);
            }
        }
    }
