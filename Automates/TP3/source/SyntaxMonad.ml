  ...
  *)
  ...
  (match (peekAtFirstToken stream) with
    ...
    | LetToken ->
      ...
      accept InToken >>=
      parseE
    (* regle #3 #3  - E -> letrec ident = E in E*)
    | RecToken ->
      inject stream >>=
      accept RecToken >>=
      acceptIdent >>=
      ...
      accept InToken >>=
      parseE
    (* regle #4   #4  - E -> if E then E else E*)
    |IfToken ->
      inject stream >>=
      accept IfToken >>=
      parseE >>=
...
and parseARG stream =
  ...
  (match (peekAtFirstToken stream) with
    (* regle 25 - ARG -> ( E ) *)
      ...
      accept RightParenthesisToken
    (* regle 26  #26 - ARG ->*)
    | (TimesToken | DivideToken | EqualToken | PlusToken | MinusToken |RightParenthesisToken) ->
      inject stream
    | EOSToken -> inject stream
    | _ -> Failure)
;;
