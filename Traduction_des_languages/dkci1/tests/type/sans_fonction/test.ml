...
let%test_unit "testAffectationTypeIncompatible" = 
  try 
    let _ = compiler (pathFichiersRat^"testAffectationTypeIncompatible.rat")
    in raise ErreurNonDetectee
  with
  | TypeInattendu(Bool, Int) -> ()

let%test_unit "testAffectableRat" = 
  let _ = compiler (pathFichiersRat^"testAffectableRat.rat") in ()
  
(* Tests des pointeurs *)

let%test_unit "testPointeurTypeCompatible" = 
  let _ = compiler (pathFichiersRat^"testPointeurTypeCompatible.rat") in ()

let%test_unit "testPointeurTypeIncompatible" = 
  try 
    let _ = compiler (pathFichiersRat^"testPointeurTypeIncompatible.rat")
    in raise ErreurNonDetectee
  with
  | TypeInattendu(Pointeur Bool, Pointeur Int) -> ()

let%test_unit "testDerefType" = 
  let _ = compiler (pathFichiersRat^"testDerefType.rat") in ()

let%test_unit "testAdresseType" = 
  let _ = compiler (pathFichiersRat^"testAdresseType.rat") in ()

let%test_unit "testNewType" = 
  let _ = compiler (pathFichiersRat^"testNewType.rat") in ()

(* Tests des types énumérés *)

let%test_unit "testEnumTypeCompatible" = 
  let _ = compiler (pathFichiersRat^"testEnumTypeCompatible.rat") in ()

let%test_unit "testEnumTypeIncompatible" = 
  try 
    let _ = compiler (pathFichiersRat^"testEnumTypeIncompatible.rat")
    in raise ErreurNonDetectee
  with
  | TypeInattendu _ -> ()

let%test_unit "testEnumEgaliteTypeDifferent" = 
  try 
    let _ = compiler (pathFichiersRat^"testEnumEgaliteTypeDifferent.rat")
    in raise ErreurNonDetectee
  with
  | TypeBinaireInattendu _ -> ()

let%test_unit "testEnumAffichage" = 
  try 
    let _ = compiler (pathFichiersRat^"testEnumAffichage.rat")
    in raise ErreurNonDetectee
  with
  | AffichageTypeInvalide _ -> ()

let%test_unit "testEnumAvecInt" = 
  try 
    let _ = compiler (pathFichiersRat^"testEnumAvecInt.rat")
    in raise ErreurNonDetectee
  with
  | TypeBinaireInattendu _ -> ()

let%test_unit "testEnumDateRenduExSujet" = 
  let _ = compiler (pathFichiersRat^"testEnumDateRenduExSujet.rat") in ()
