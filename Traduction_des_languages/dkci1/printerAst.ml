...
struct
  ...
  let rec string_of_instruction i =
    ...
    | RetourVide -> "Retour  : RETURN\n"
    | AppelProc (n,le) -> "AppelProc  : call "^n^"("^((List.fold_right (fun i tq -> (string_of_expression i)^tq) le ""))^");\n"

  (* Conversion d'une définition d'enum *)
  let string_of_enum_def (EnumDef (nom, valeurs)) =
    "enum " ^ nom ^ " { " ^ (String.concat ", " valeurs) ^ " };\n"

  (* Conversion des fonctions *)
  let string_of_fonction (Fonction(t,n,lp,li)) = 
  (string_of_type t)^" "^n^" ("^
  ((List.fold_right (fun (tp, is_ref, np) tq -> 
    (if is_ref then "ref " else "") ^ (string_of_type tp)^" "^np^" "^tq
  ) lp ""))^
  ") = \n"^
  ...
  let string_of_programme (Programme (enums, fonctions, instruction)) =
    (*ajout du paramètre enums *)
    (List.fold_right (fun e tq -> (string_of_enum_def e)^tq) enums "")^
