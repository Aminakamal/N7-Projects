...
//                PRINTVAR( line );
        ...
        if ( line[0] == 'f' )
        {
            ...
            //*********************************************************************
            const auto n = computeNormal(vertices[t.v1], vertices[t.v2], vertices[t.v3]);

            //*********************************************************************
            // Sum the normal of the face to each vertex normal (to be done for section 5.3)
            //*********************************************************************
            float angleV1 = angleAtVertex(vertices[t.v1], vertices[t.v2], vertices[t.v3]);
            float angleV2 = angleAtVertex(vertices[t.v2], vertices[t.v1], vertices[t.v3]);
            float angleV3 = angleAtVertex(vertices[t.v3], vertices[t.v1], vertices[t.v2]);

            normals[t.v1] += n*angleV1;
            normals[t.v2] += n*angleV2;
            normals[t.v3] += n*angleV3;
