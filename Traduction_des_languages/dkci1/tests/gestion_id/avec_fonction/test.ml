...
(* Les tests des affectables *)

let%test_unit "testAffectationFonction" = 
  try 
    let _ = compiler (pathFichiersRat^"testAffectationFonction.rat")
    in raise ErreurNonDetectee
  with
  | MauvaiseUtilisationIdentifiant("f") -> ()

let%test_unit "testAffectableParametre" = 
  let _ = compiler (pathFichiersRat^"testAffectableParametre.rat") in ()
  
(* Les tests des pointeurs *)

let%test_unit "testPointeurParametre" = 
  let _ = compiler (pathFichiersRat^"testPointeurParametre.rat") in ()

let%test_unit "testPointeurDouble" = 
  let _ = compiler (pathFichiersRat^"testPointeurDouble.rat") in ()
  
(* Les tests des PROCÉDURES*)

let%test_unit "testProcedureAvecParam" = 
  let _ = compiler (pathFichiersRat^"testProcedureAvecParam.rat") in ()

let%test_unit "testErreurRetourProc" = 
  try 
    let _ = compiler (pathFichiersRat^"testErreurRetourProc.rat")
    in raise ErreurNonDetectee
  with
  | RetourAvecValeurDansVoid -> ()


(* Tests Ref*)

let%test_unit "testRefBasique" = 
  let _ = compiler (pathFichiersRat ^ "testRefBasique.rat") in ()

let%test_unit "testRefRat" = 
  let _ = compiler (pathFichiersRat ^ "testRefRat.rat") in ()

let%test_unit "testRefMultiples" = 
  let _ = compiler (pathFichiersRat ^ "testRefMultiples.rat") in ()

let%test_unit "testRefMixte" = 
  let _ = compiler (pathFichiersRat ^ "testRefMixte.rat") in ()

let%test_unit "testRefSurConstante" = 
  try
    let _ = compiler (pathFichiersRat ^ "testRefSurConstante.rat") in
    raise ErreurNonDetectee
  with
  | MauvaiseUtilisationIdentifiant "c" -> ()

let%test_unit "testRefSurFonction" = 
  try
    let _ = compiler (pathFichiersRat ^ "testRefSurFonction.rat") in
    raise ErreurNonDetectee
  with
  | MauvaiseUtilisationIdentifiant "g" -> ()
let%test_unit "testRefNonDeclare" = 
  try
    let _ = compiler (pathFichiersRat ^ "testRefNonDeclare.rat") in
    raise ErreurNonDetectee
  with
  | IdentifiantNonDeclare "x" -> ()
let%test_unit "testRefRecursif" = 
  let _ = compiler (pathFichiersRat ^ "testRefRecursif.rat") in ()

let%test_unit "testRefChaine" = 
  let _ = compiler (pathFichiersRat ^ "testRefChaine.rat") in ()
  
(* Tests des types énumérés avec fonctions *)
...
let%test_unit "testEnumProcedureParam" = 
  let _ = compiler (pathFichiersRat^"testEnumProcedureParam.rat") in ()

let%test_unit "testEnumRefParam" = 
  let _ = compiler (pathFichiersRat^"testEnumRefParam.rat") in ()

let%test_unit "testEnumRefErreur" = 
  try 
    let _ = compiler (pathFichiersRat^"testEnumRefErreur.rat")
    in raise ErreurNonDetectee
  with
